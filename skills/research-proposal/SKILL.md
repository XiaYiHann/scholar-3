...
description: 进入 Proposal 模式（命令入口 `/proposal`）- 在单一冻结 Idea 下创建分阶段 Proposal（plan + tasks），并准备执行。
version: 2.0.0
metadata:
  phase: proposal
  outputs:
...
进入 Proposal 模式（本阶段首先是：**创建/维护分阶段 proposal 工件**）。
...
- 必须存在：
  ...
  - `research/evidence-ledger.md`
- 必须先读取 `research/idea.md` 中的 **Proposal Handoff Capsule**（若缺失，先回 `/discover` 补齐）
...
- proposal.md 必须显式引用 handoff capsule 的：
  - locked metric/tuning
  - baseline set
  - top risks
  - open blockers
