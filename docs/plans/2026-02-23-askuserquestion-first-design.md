...
**日期**: 2026-02-23  
**范围**: `scholar-3` 三阶段工作流（`/discover`、`/experiment`、`/paper`）  

## 背景与问题

当前三阶段工作流主要依赖用户自由输入。对“意图澄清、分岔选择、参数收集”等场景，用户需要自己组织文本回答，导致：
...
- 在三阶段全流程中，尽可能用结构化提问替代自由输入（必要时仍允许 `Other` 文本兜底）
...
- 不新增总入口命令（如 `/scholar`）；本次仅优化三个阶段命令/技能内部交互
...
4. 问题 `id` 使用 `snake_case`，带阶段前缀（`discover_*`/`exp_*`/`paper_*`）
...
- `exp_input`（材料）: 有 idea-proposal.md / 只有口头描述 / 想先回 discover
- `exp_focus`（重点）: 实验设计 / 代码实现与可复现 / 分析已有结果
- `exp_rigor`（可复现）: 标准可复现 / 快速跑通优先 / 严格记录
...
- `/experiment`: 先确认输入材料与本次重点，再决定是否调用 `architect`、是否进入编码或仅输出设计/分析；所有实验参数（框架、数据集、指标、baseline 范围）优先结构化收集
- `/paper`: 根据写作状态与本次优先输出，决定生成 `research/manuscript.md` 还是 `templates/rebuttal.md` 路径；先结构化确认 venue/篇幅/重点后再开始长文本生成

## 验证方式

- 在 Claude Code 与 Codex 中分别运行 `/discover`、`/experiment`、`/paper` 观察：
  - 是否首先出现入口问诊（选项式）
  - 是否在关键分岔点继续使用选项式追问
